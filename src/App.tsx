...
function App() {
  ...
  // Setter only — retranscribingNoteId tracked for future NoteView banner
  const [, setRetranscribingNoteId] = useState<string | null>(null);
  ...
  const handleContextMenuAction = (action: string) => {
    ...
    } else if (action === "rename" && contextMenu?.noteId) {
      const note = notes.find((n) => n.id === contextMenu.noteId);
      if (note) {
  ...
  const handleRenameCommit = async (noteId: string) => {
    if (renameValue.trim()) {
  ...
  return (
    <AuthGuard onShowLogin={() => setAuthScreen("login")}>
      ...
      >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            <span
              style={{
                ...
                background:
                  "var(--color-bg-subtle, var(--color-sidebar-hover))",
                padding: "2px 8px",
                borderRadius: "4px",
              }}
            >
          ...
          >
                      ...
                      >
                        {renamingNoteId === note.id ? (
                          <input
                            ...
                            onBlur={() => handleRenameCommit(note.id)}
                            onKeyDown={(e) => {
                              if (e.key === "Enter")
                                handleRenameCommit(note.id);
                              if (e.key === "Escape") {
                                setRenamingNoteId(null);
                                setRenameValue("");
                              }
                            ...
                            className="text-sm font-medium w-full bg-transparent border-b outline-none"
                            style={{
            ...
            >
              ...
              <button
                onClick={() =>
                  setSelectedSection(
                    selectedSection === "my-sessions" ? "notes" : "my-sessions"
                  )
                }
              ...
              {isAdmin() && (
                <button
                  onClick={() =>
                    setSelectedSection(
                      selectedSection === "admin" ? "notes" : "admin"
                    )
                  }
                  className="w-full flex items-center gap-2 p-2 rounded-lg hover:bg-black/5 transition-colors mb-1"
                  style={{
              ...
              <NoteView
                ...
                onContinueRecording={async () => {
                  try {
                    ...
                    const existingSegments = await loadTranscript(
                      selectedNote.id
                    );
                ...
                onExport={async () => {
                  try {
                    ...
                    const filePath = await exportApi.savePdfWithDialog(
                      data.markdown,
                      data.filename
                    );
                    if (filePath) {
        ...
        {/* end main content flex row */}
      </div>
      {/* end outer column flex */}
    </AuthGuard>
  );
}
    ...
    >
      <div className="text-center px-8" style={{ maxWidth: 420 }}>
        ...
        <p
          ...
          style={{ color: "var(--color-text-secondary)" }}
        >
          AI-powered meeting assistant — transcribe, summarize, and capture key
          points.
        </p>
        ...
        {needsSetup && (
          ...
          >
            ...
            Complete setup — Whisper & Ollama required
          </button>
        )}
      </div>
    </div>
          ...
          <div className="flex-1 min-w-0">
            {editingTitle ? (
              <input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && onUpdateTitle(titleValue)
                }
          ...
          <div className="flex items-center gap-1.5">
            ...
            {!isRecording && !isPaused && !isRegenerating && !isGenerating && (
              <>
                ...
                {loadedModel &&
                  (audioSegments.length > 0 || uploads.length > 0) && (
                    <button
